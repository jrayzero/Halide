...
WEAK work_64bit_queue_t work_64bit_queue;

      WEAK void worker_64bit_thread_already_locked(work_64bit *owned_job) {
    // If I'm a job owner, then I was the thread that called
    // do_par_for, and I should only stay in this function until my
    // job is complete. If I'm a lowly worker thread, I should stay in
    // this function as long as the work queue is running.
    while (owned_job != NULL ? owned_job->running()
           : work_64bit_queue.running()) {

        if (work_64bit_queue.jobs == NULL) {
            if (owned_job) {
                // There are no jobs pending. Wait for the last worker
                // to signal that the job is finished.
                halide_cond_wait(&work_64bit_queue.wakeup_owners, &work_64bit_queue.mutex);
            } else if (work_64bit_queue.a_team_size <= work_64bit_queue.target_a_team_size) {
                // There are no jobs pending. Wait until more jobs are enqueued.
                halide_cond_wait(&work_64bit_queue.wakeup_a_team, &work_64bit_queue.mutex);
            } else {
                // There are no jobs pending, and there are too many
                // threads in the A team. Transition to the B team
                // until the wakeup_b_team condition is fired.
                work_64bit_queue.a_team_size--;
                halide_cond_wait(&work_64bit_queue.wakeup_b_team, &work_64bit_queue.mutex);
                work_64bit_queue.a_team_size++;
            }
        } else {
            // Grab the next job.
            work_64bit *job = work_64bit_queue.jobs;

            // Claim a task from it.
            work_64bit myjob = *job;
            job->next++;

            // If there were no more tasks pending for this job,
            // remove it from the stack.
            if (job->next == job->max) {
                work_64bit_queue.jobs = job->next_job;
            }
            ...
            // Release the lock and do the task.
            halide_mutex_unlock(&work_64bit_queue.mutex);
            int result = halide_64bit_do_task(myjob.user_context, myjob.f, myjob.next,
                                        myjob.closure);
            halide_mutex_lock(&work_64bit_queue.mutex);
            ...
            // the owner.
            if (!job->running() && job != owned_job) {
                halide_cond_broadcast(&work_64bit_queue.wakeup_owners);
            }
        }
    }
    }

WEAK void worker_64bit_thread(void *) {
    halide_mutex_lock(&work_64bit_queue.mutex);
    worker_64bit_thread_already_locked(NULL);
    halide_mutex_unlock(&work_64bit_queue.mutex);
}
...
WEAK int halide_64bit_default_do_par_for(void *user_context, halide_64bit_task_t f,
    ...
    if (!work_64bit_queue.initialized) {
        work_64bit_queue.shutdown = false;
        halide_cond_init(&work_64bit_queue.wakeup_owners);
        halide_cond_init(&work_64bit_queue.wakeup_a_team);
        halide_cond_init(&work_64bit_queue.wakeup_b_team);
        work_64bit_queue.jobs = NULL;
        ...
        // Everyone starts on the a team.
        work_64bit_queue.a_team_size = work_64bit_queue.desired_num_threads;

        work_64bit_queue.initialized = true;
    }

    while (work_64bit_queue.threads_created < work_64bit_queue.desired_num_threads - 1) {
        // We might need to make some new threads, if work_64bit_queue.desired_num_threads has
        // increased.
        work_64bit_queue.threads[work_64bit_queue.threads_created++] =
            halide_spawn_thread(worker_64bit_thread, NULL);
    }
    ...
    if (!work_64bit_queue.jobs && size < (size_t)work_64bit_queue.desired_num_threads) {
        // If there's no nested parallelism happening and there are
        // fewer tasks to do than threads, then set the target A team
        // size so that some threads will put themselves to sleep
        // until a larger job arrives.
        work_64bit_queue.target_a_team_size = size;
    } else {
        // Otherwise the target A team size is
        // desired_num_threads. This may still be less than
        // threads_created if desired_num_threads has been reduced by
        // other code.
        work_64bit_queue.target_a_team_size = work_64bit_queue.desired_num_threads;
    }

    // Push the job onto the stack.
    job.next_job = work_64bit_queue.jobs;
    work_64bit_queue.jobs = &job;

    // Wake up our A team.
    halide_cond_broadcast(&work_64bit_queue.wakeup_a_team);

    // If there are fewer threads than we would like on the a team,
    // wake up the b team too.
    if (work_64bit_queue.target_a_team_size > work_64bit_queue.a_team_size) {
        halide_cond_broadcast(&work_64bit_queue.wakeup_b_team);
    }

    // Do some work myself.
    worker_64bit_thread_already_locked(&job);
...
WEAK int halide_64bit_set_num_threads(int n) {
    if (n < 0) {
        halide_error(NULL, "halide_set_num_threads: must be >= 0.");
    }
    // Don't make this an atomic swap - we don't want to be changing
    ...
    int old = work_64bit_queue.desired_num_threads;
    work_64bit_queue.desired_num_threads = clamp_num_threads(n);
    halide_mutex_unlock(&work_64bit_queue.mutex);
    return old;
}

WEAK void halide_shutdown_thread_pool() {
    if (!work_64bit_queue.initialized) return;

    // Wake everyone up and tell them the party's over and it's time
    // to go home
    halide_mutex_lock(&work_64bit_queue.mutex);
    work_64bit_queue.shutdown = true;
    halide_cond_broadcast(&work_64bit_queue.wakeup_owners);
    halide_cond_broadcast(&work_64bit_queue.wakeup_a_team);
    halide_cond_broadcast(&work_64bit_queue.wakeup_b_team);
    halide_mutex_unlock(&work_64bit_queue.mutex);
    ...
    // Wait until they leave
    for (int i = 0; i < work_64bit_queue.threads_created; i++) {
        halide_join_thread(work_64bit_queue.threads[i]);
    }

    // Tidy up
    halide_mutex_destroy(&work_64bit_queue.mutex);
    halide_cond_destroy(&work_64bit_queue.wakeup_owners);
    halide_cond_destroy(&work_64bit_queue.wakeup_a_team);
    halide_cond_destroy(&work_64bit_queue.wakeup_b_team);
    work_64bit_queue.initialized = false;
}
