...
class TrimNoOps : public IRMutator {
    ...
    void visit(const For *op) {
        ...
        Expr new_min_var = Variable::make(op->min.type(), new_min_name);
        Expr new_max_var = Variable::make(op->min.type(), new_max_name);
        Expr old_max_var = Variable::make(op->min.type(), old_max_name);
