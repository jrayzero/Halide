...
void CodeGen_GPU_Host<CodeGen_CPU>::visit(const For *loop) {
    if (CodeGen_GPU_Dev::is_gpu_var(loop->name)) {
        ...
        Value *launch_args[] = {
            ...
            entry_name_str,
            codegen(cast(Int(32), bounds.num_blocks[0])), 
            codegen(cast(Int(32), bounds.num_blocks[1])), 
            codegen(cast(Int(32), bounds.num_blocks[2])), 
            codegen(cast(Int(32), bounds.num_threads[0])), 
            codegen(cast(Int(32), bounds.num_threads[1])), 
            codegen(cast(Int(32), bounds.num_threads[2])), 
